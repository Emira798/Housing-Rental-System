...
const avatarUrl = computed(() => {
  // 如果没有头像或头像路径为空，返回null让el-avatar显示默认图标
  if (!userForm.avatar || userForm.avatar.trim() === '') {
    return null;
  }
  // 确保头像路径以/开头
  const avatarPath = userForm.avatar.startsWith('/') ? userForm.avatar : `/${userForm.avatar}`;
  ...
  try {
    ...
    if (userStore.isLoggedIn) {
      ...
      if (response) {
        ...
        userForm.idCard = response.idCard || "";
